...
\lstset{
    ...
    tabsize=4,
    keywordstyle=\color{mfpurple},
    commentstyle=\color{mfgreen},
...
\caption*{\small\color{mfgray} Metaflow UI.}
\end{minipage}
\hfill
...
Eight utility functions, each with version-gated smart routing:
\begin{itemize}[nosep]
    ...
    \item \texttt{compare\_runs()}: diff two runs by step timing, task counts, failure rates
    \item \texttt{detect\_regressions()}: find steps whose duration increased across recent runs
\end{itemize}
...
The extension package alone helps Python users. To make Metaflow truly \textit{agent-friendly}, the utilities need to be discoverable by any LLM agent without custom integration. We build an MCP (Model Context Protocol) server that exposes all 8 utility functions as tools:

\begin{lstlisting}[language=Python]
# Any MCP-compatible agent discovers and calls these automatically
{"name": "find_failures",
 "description": "Find failed tasks in a Metaflow run",
 "parameters": {"run_pathspec": "str", "max_tasks": "int=100"},
 "cost": "2-3 HTTP calls (fast path), O(tasks) calls (fallback)"}
\end{lstlisting}

Each tool schema includes cost annotations so agents can reason about efficiency. System prompt fragments guide agents toward bounded queries. The MCP server auto-detects available infrastructure (metadata service version, UI Backend presence) and routes queries through the optimal path.

\subsection{Agent Simulation Framework}

Not a one-off benchmark but a reusable framework with 5 scripted scenarios that measure real agent behavior:

\begin{enumerate}[nosep]
    \item ``Which of my last 10 runs failed?'' (run filtering)
    \item ``What tasks failed and what were their logs?'' (failure diagnosis)
    \item ``Show runs from the last 24h that took > 5 minutes'' (time-range query)
    \item ``Compare this run to the previous successful one'' (cross-run diff)
    \item ``Is this step getting slower over time?'' (regression detection)
\end{enumerate}

Each scenario runs against both the naive Client API and the extension package, measuring HTTP call counts, latency, and bytes transferred. The framework doubles as a regression test: if a code change increases call counts, the simulation catches it.

\subsection{Cross-run Analytics}

Queries that don't exist today and require new thinking, not just wrapping existing endpoints:

\begin{itemize}[nosep]
    \item \texttt{compare\_runs(run\_a, run\_b)}: structured diff of step timings, task counts, failure rates, artifact sizes. Answers ``what changed between these two runs?''
    \item \texttt{detect\_regressions(flow, n=10)}: analyze the last N runs to find steps whose median duration increased significantly. Answers ``is this step getting slower?''
\end{itemize}

These are the kinds of questions agents ask repeatedly when debugging production workflows. Today, answering them requires dozens of API calls assembled manually. The extension package makes them single-function calls with bounded cost.
...
\midrule
1 (Wk 1--4)   & Extension package: 8 utility functions + cross-run analytics, unit + integration tests & 90 & Midterm checkpoint \\
2 (Wk 5--8)   & Metadata service \texttt{\_limit}/\texttt{\_order} + provider version-gating + caching & 90 & Core changes ready \\
3 (Wk 9--12)  & MCP tool server, agent simulation framework (5 scenarios), robustness testing & 90 & End-to-end validated \\
4 (Wk 13--16) & User + agent docs, PR prep per CONTRIBUTING.md, final report & 80 & Submitted \\
\bottomrule
\end{tabularx}

\textbf{Midterm checkpoint (Week 4):} Working extension package with 8 functions (including cross-run analytics), all tested, benchmark showing improvement over the naive path. Each phase delivers standalone value: Phase 1 alone is a complete, usable deliverable.
